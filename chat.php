...
// (تم إصلاح هذا الشرط) — سابقًا كان يقارن بنص لا يطابق القيمة الفعلية
// في config.php أبدًا، فكان يفوّت حالة المفتاح الفارغ ويكمل الطلب فيفشل لاحقًا.
if (
...
// (تم إصلاح هذا) — gemini-2.0-flash تم إيقافه من جوجل (31 مارس 2026)
// واستبداله بموديل حالي مدعوم.
...
if (!function_exists('curl_init')) {
    http_response_code(500);
    echo json_encode(['error' => 'إضافة cURL غير مفعّلة على السيرفر. فعّلها من لوحة تحكم الاستضافة (PHP Extensions).']);
    exit;
}
...
// إذا واجهت هذا الخطأ فقط، فعّل السطرين التاليين مؤقتًا (غير موصى به على المدى الطويل):
